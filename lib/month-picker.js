...
//     const keys = __VKEYS
//     const items = keys.concat([ 'choice', 'from', 'to', ])
    ...
    _createClass(MonthPicker, [{
        ...
        value: function optionPad(padIndex) {
            ...
            if (rawValue.type === 'single') {
                ...
                isRange = true;
                if (!labelYear) {
                    labelYear = labelYears[padIndex] = rawValue[['from', 'to'][padIndex]].year;
                }
                ...
                values.isStartYear = from.year === labelYear;
                values.isEndYear = to.year === labelYear;
            }
            ...
            var otherValue = false,
                labelPreText = void 0;
            if (isRange) {
                otherValue = rawValue[__VKEYS[1 - padIndex]];
                labelPreText = _react2.default.createElement(
                    'b',
                    null,
                    this.props.lang[__VKEYS[padIndex]]
                );
            }
            ...
            return _react2.default.createElement(
                ...
                _react2.default.createElement(
                    ...
                    mapToArray(12, function (i) {
                        ...
                        // }
                        // if (isRange) {
                        //     if (padIndex === 0 && (labelYear > value.year || (labelYear === value.year && m > value.month))) {
                        //         css = 'select'
                        //     }
                        //     if (padIndex === 1 && (labelYear < value.year || (labelYear === value.year && m < value.month))) {
                        //         css = 'select'
                        //     }
                        // }
                        ...
                        if (atMinYear && m < ymArr[0].min) {
                            ...
                            for (var _i = valOffset, last = values.length - 1; _i <= last; _i++) {
                                ...
                                if (v === m) {
                                    ...
                                    if (!isRange) {
                                        css = 'active';
                                    } else if (values.isStartYear && _i === 0) {
                                        css = 'select';
                                    } else if (values.isEndYear && _i === last) {
                                        css = 'select';
                                    }
                                }
                            }
    ...
    this._handleClickMonth = function (e) {
        if (_this4.state.showed) {
            ...
            if (rawValue.type === 'single') {
                ...
                var keys = ['from', 'to'];
                rawValue[keys[idx]] = { year: year, month: month };
            }
            _this4.setState({ rawValue: rawValue });
